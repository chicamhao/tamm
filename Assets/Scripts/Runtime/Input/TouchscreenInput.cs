...
{
    public sealed class TouchscreenInput : MonoBehaviour
    {
        [Header("Settings")]
        ...
        private void Start()
        {
            var joystickMove = _document.rootVisualElement.Q<VisualElement>("JoystickMove");
            var joystickLook = _document.rootVisualElement.Q<VisualElement>("JoystickLook");

            _moveJoystick = new VirtualJoystick(joystickMove);
            _moveJoystick.JoystickEvent.AddListener(mov =>
            {
                MoveEvent?.Invoke(mov * MoveMagnitudeMultiplier);
            });

            _lookJoystick = new VirtualJoystick(joystickLook);
            _lookJoystick.JoystickEvent.AddListener(mov =>
            {
                if (InvertLookY)
                    mov.y *= -1;
            ...
            var jumpButton = _document.rootVisualElement.Q<VisualElement>("ButtonJump");
            ...
            var sprintButton = _document.rootVisualElement.Q<VisualElement>("ButtonSprint");
