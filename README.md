...
3. Open `Playground` and press Play. Settings gear → Save / Load / New Game; **F2** opens
   the dev chapter stepper.

## The invariant

`Services.*` is never null at runtime — there are **no null guards** on service access.
`Playground.unity` is the persistent **core scene**: the `Bootstrapper` GameObject is
`DontDestroyOnLoad` and owns every service; level scenes hold only content. Never call
`SceneManager.LoadScene` directly — use `Bootstrapper.LoadLevel("...")`
